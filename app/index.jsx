...
// Choisit un index aléatoire en excluant obligatoirement tous les exclus.
// Si le pool est plus petit que les exclusions, on réinitialise pour ne garder
// que le dernier vu (évite le blocage infini).
function getRandomFrom(pool, excludeIndexes = []) {
  // Si on a tout exclu, on ne garde que le dernier pour libérer les autres
  let excluded = excludeIndexes;
  if (excluded.length >= pool.length) {
    excluded = [excludeIndexes[excludeIndexes.length - 1]];
  }

  // Construit la liste des candidats disponibles
  const candidates = pool
    .map((_, i) => i)
    .filter((i) => !excluded.includes(i));

  // Tire un candidat au hasard parmi ceux disponibles
...
function CamilleTab() {
  ...
  // lastShown stocke l'index actuellement affiché pour chaque catégorie
  ...
  const handleGenerate = async () => {
    ...
    // Charge l'historique persistant (5 derniers tirages par catégorie)
    let history = {};
    try {
    ...
    for (const cat of categoriesContenu.filter((c) => c.id !== "message")) {
      const pool = POOLS[cat.id];
      // Combine l'historique persistant + l'index actuellement affiché
      const currentlyShown = lastShownRef.current[cat.id];
      const persistedExcluded = history[cat.id] || [];
      const excluded = currentlyShown !== undefined
        ? [...new Set([...persistedExcluded, currentlyShown])]
        : persistedExcluded;

      const { item, index } = getRandomFrom(pool, excluded);
      result[cat.id] = { item, index, cat };
      history[cat.id] = [...persistedExcluded, index].slice(-5);
    }

    // Message complet
    const msgPool = poolMessages;
    const currentMsg = lastShownRef.current["message"];
    const msgPersisted = history["message"] || [];
    const msgExcluded = currentMsg !== undefined
      ? [...new Set([...msgPersisted, currentMsg])]
      : msgPersisted;

    const { item: msgItem, index: msgIdx } = getRandomFrom(msgPool, msgExcluded);
    result["message"] = { item: msgItem, index: msgIdx, cat: categoriesContenu.find((c) => c.id === "message") };
    history["message"] = [...msgPersisted, msgIdx].slice(-5);

    // Met à jour lastShownRef avec ce qui vient d'être tiré
    lastShownRef.current = {
      surnom:     result.surnom?.index,
      compliment: result.compliment?.index,
      blague:     result.blague?.index,
      sujet:      result.sujet?.index,
      message:    msgIdx,
    };

    // Sauvegarde persistante
    try {
      await AsyncStorage.setItem("camille_history", JSON.stringify(history));
    } catch (e) {}
  ...
  return (
    <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
      ...
      <View style={camS.heroCard}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={camS.heroSub}>
            Des mots justes, des taquineries, des sujets – générés depuis votre conversation.
          </Text>
        </View>
      </View>
      ...
      <TouchableOpacity
        ...
        disabled={loading}
      >
        <Text style={camS.generateBtnEmoji}>{loading ? "⏳" : "✨"}</Text>
      ...
      {generated && (
        <Animated.View style={{ opacity: fadeAnim, transform: [{ scale: scaleAnim }] }}>
          {/* Surnom */}
          <ResultCard
            cat={generated.surnom.cat}
            content={generated.surnom.item}
            onCopy={() => handleCopy(generated.surnom.item, "surnom")}
            copied={copied === "surnom"}
          />

          {/* Compliment */}
          <ResultCard
            cat={generated.compliment.cat}
            content={generated.compliment.item}
            onCopy={() => handleCopy(generated.compliment.item, "compliment")}
            copied={copied === "compliment"}
          />

          {/* Blague */}
          <ResultCard
            cat={generated.blague.cat}
            content={generated.blague.item}
            onCopy={() => handleCopy(generated.blague.item, "blague")}
            copied={copied === "blague"}
          />

          {/* Sujet de conversation */}
          <ResultCard
            cat={generated.sujet.cat}
            content={generated.sujet.item}
            onCopy={() => handleCopy(generated.sujet.item, "sujet")}
            copied={copied === "sujet"}
          />

          {/* Message complet */}
          <View style={[camS.resultCard, { borderColor: generated.message.cat.couleur + "60" }]}>
            <View style={camS.resultHeader}>
              <View style={[camS.resultBadge, { backgroundColor: generated.message.cat.couleur + "20" }]}>
                <Text style={camS.resultBadgeEmoji}>{generated.message.cat.emoji}</Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[camS.resultLabel, { color: generated.message.cat.couleur }]}>
                  {generated.message.cat.label}
                </Text>
                <View style={[camS.tagPill, { backgroundColor: generated.message.cat.couleur + "20" }]}>
                  <Text style={[camS.tagText, { color: generated.message.cat.couleur }]}>
                    {generated.message.item.tag}
                  </Text>
                </View>
              </View>
              <TouchableOpacity
                style={[camS.copyBtn, copied === "message" && { backgroundColor: "#1A3D2B" }]}
                onPress={() => handleCopy(generated.message.item.texte, "message")}
                activeOpacity={0.7}
              >
            ...
            <Text style={camS.resultText}>{generated.message.item.texte}</Text>
          </View>
          ...
          <View style={camS.tipBox}>
            ...
            <Text style={camS.tipText}>
              Ces suggestions sont tirées de l'analyse de vos échanges. Adapte-les à ta façon de parler — ce sont des idées, pas des scripts.
            </Text>
          </View>
        </Animated.View>
      )}
      ...
      {!generated && (
        <View style={camS.emptyState}>
          ...
          <Text style={camS.emptyText}>
            Appuie sur "Générer" pour obtenir un surnom, un compliment, une taquinerie, un sujet de conversation et un message complet — personnalisés pour Camille.
          </Text>
        </View>
      )}
...
function ResultCard({ cat, content, onCopy, copied }) {
  return (
    <View style={[camS.resultCard, { borderColor: cat.couleur + "55" }]}>
      <View style={camS.resultHeader}>
        ...
        <Text style={[camS.resultLabel, { color: cat.couleur, flex: 1 }]}>{cat.label}</Text>
        <TouchableOpacity
